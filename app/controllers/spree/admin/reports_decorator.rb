...
module Spree
  module Admin
    ReportsController.class_eval do
      ...
      def monthly_revenue
        render json: {}.tap {|r| Spree::Order.all.group_by {|u| u.created_at.strftime('%B')}.collect {|k, v| {k => v.map!(&:total).sum.to_i}}.each{ |h| h.each{|k,v| r[k] = v } }}
      end

      def monthly_tax
        render json: {}.tap {|r| Spree::Order.all.group_by {|u| u.created_at.strftime('%B')}.collect {|k, v| {k => v.map!(&:included_tax_total).sum.to_i}}.each {|h| h.each{|k,v| r[k] = v}}}
      end

      def new_users_by_date
        render json: {}.tap {|r| Spree::User.all.group_by {|u| u.created_at.strftime('%B')}.sort.collect {|k, v| {k => v.length}}.each{ |h| h.each{|k,v| r[k] = v } }}

        # Spree::User.all.group(:created_at).count
      end

      def products_by_category
        # Spree::Product.all.group(:price).count
      end

      def interest_groups
        render json: {}.tap {|r| Spree::Interest.all.collect {|i| {i.interest => i.users.length}}.each{ |h| h.each{|k,v| r[k] = v }}}
      end

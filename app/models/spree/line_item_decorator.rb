# From spree core models line items
...
module Spree
  LineItem.class_eval do
    ...
    def copy_price
      if variant
        self.price = variant.price_in if price.nil?
    ...
    def single_money
      ...
      # Spree::Money.new(product.price_in(currency, @current_store.id).display_price, { currency: currency, store: @current_store })
    end
    ...
    def options=(options={})
      ...
      store = opts.delete(:store) || order.try(:store) || Spree::Store.current || Spree::Store.default.id
      # role = current_user

      if currency
        self.currency = currency
        # TODO have modified
        self.price    = variant.price_in(currency, store, nil).amount +
